...
router.get('/equipe/:idUsuario', function (req, res, next) {
    ...
    var idUsuario = req.params.idUsuario;
    let instrucaoSql = `select * from  [dbo].[Leitura] inner join [dbo].[Equipamento] on fkEquipamento = idEquipamento inner join [dbo].[Funcionario] on fkFuncionario = idFuncionario`;
    sequelize.query(instrucaoSql, {
        model: Publicacao,
        mapToModel: true
    })
